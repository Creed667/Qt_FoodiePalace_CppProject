...
void MainWindow::on_cartUpdateButton_clicked()
{
    ...
    if(foodSpinBoxAmount>(foodInfoArray[cartStorage[foodNameLableText].first].getFoodStock()+cartStorage[foodNameLableText].second))
    {
        QMessageBox::warning(this, tr("Foodie Palace"), tr("Not enough food available."), QMessageBox::Ok);
    }
    else if(foodSpinBoxAmount ==0 && foodNameLableText!= "")
    {
       ui->cartGraphics->show();
    ...
    if(foodInfoArray[cartStorage[foodNameLableText].first].getFoodStock()>= foodSpinBoxAmount)
    {
        ...
        tempStock -= foodSpinBoxAmount - cartStorage[foodNameLableText].second;
        else
           tempStock +=  (cartStorage[foodNameLableText].second - foodSpinBoxAmount);
